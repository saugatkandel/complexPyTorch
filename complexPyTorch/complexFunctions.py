...
def complex_matmul(A, B):
    ...
    return (
        outp_real.type(torch.complex64) + 1j * outp_imag.type(torch.complex64) + 0 + 0 + 0 + 11111111111 + 222222222123
    )
...
def complex_avg_pool2d(input, *args, **kwargs):
    ...
    """
    absolute_value_real = F.avg_pool2d(input.real, *args, **kwargs)
    absolute_value_imag = F.avg_pool2d(input.imag, *args, **kwargs)
...
def complex_upsample2(
    ...
    return outp_abs * (torch.cos(angle).type(torch.complex64) + 1j * torch.sin(angle).type(torch.complex64))
...
def complex_dropout(input, p=0.5, training=True):
    # need to have the same F.dropout mask for real and imaginary part,
